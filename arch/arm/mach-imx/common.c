...
void __init imx6_enet_mac_init(const char *enet_compat, const char *ocotp_compat)
{
        ...
        system_serial_low = imx_ocotp_read(OCOTP_CFG0);
        system_serial_high = imx_ocotp_read(OCOTP_CFG1);

        // printk(KERN_INFO "Serial\t\t: %08x %08x\n",system_serial_high, system_serial_low);

        if( !imx_opotp_check (system_serial_low, system_serial_high))
        {
                machine_halt();
                return;
        }
